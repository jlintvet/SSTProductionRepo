...
- **`TimeScrubber.jsx`** (shared component, used by both Radar and Wind): play/pause + scrub bar rendered at the bottom of the map when `showRadarOverlay && radarFrames.length`. Stacks above the wind time slider if both happen to be active (`bottomOffset` prop). Renders its own `<input type=range>` CSS per instance (`.ts-<accentColor>-range` class, defined inline via a `<style>` tag, unique per accent color) — the unstyled default track is nearly invisible against the dark bar. On mobile (<640px), it adds 56px of bottom clearance (`avoidMobileWeatherSheet` prop, on by default) so it sits above `WeatherBottomSheet`'s always-present 56px "peek" bar — see "Mobile weather-sheet z-index collision" below.
...
- Both desktop and mobile. Desktop: `MapControlPanel.jsx`, Tools section — Pro-gated (`ProGate`) `ToolBtn` labeled "Radar", sits directly below "Shaded Relief". Help button (`hbtn("radar")`) alongside it. Mobile: `SSTHeatmapLeaflet.jsx`'s Tools bottom-sheet drawer (`mobilePanel === "tools"`), paired with Bottom Features in a 2-column grid row, `MobileProGate`-wrapped (added `82b2006`, since the initial Pro-gating pass only touched the desktop panel).
- **Help config:** `HELP_CONFIG.radar` reuses no dedicated image (`/help/radar.png` doesn't exist — 404s silently, text-only popup).

### Mobile weather-sheet z-index collision (fixed `5ad7c57`)

Radar's slider rendered at `z-600` (see the site's documented z-order scale, `TopBar.jsx`), but `WeatherBottomSheet` is pinned to the bottom on mobile at `z-1000` with a minimum 56px "peek" height that's always present — the peek bar sat on top of the bottom portion of the slider (the play button + track), leaving only the time tooltip pill visible above it. Wind's slider is desktop-only (`isDesktop` gate) so it never hit this. Fixed by consolidating `WindTimeSlider.jsx` + `RadarTimeSlider.jsx` into `TimeScrubber.jsx` with an `avoidMobileWeatherSheet` prop that adds 56px of bottom clearance below the 640px breakpoint (matches `WeatherBottomSheet`'s own `sm:hidden` breakpoint). If a future slider or bottom-pinned element gets added, check it against this same peek bar.
...
- Color scheme and glass/floating visual treatment were both raised by Jon on 2026-07-14; the mobile z-index bug was fixed but the visual styling (blue color scheme, edge-to-edge "frosted slab" look) was explicitly left as-is per his call — revisit both together if raised again.
