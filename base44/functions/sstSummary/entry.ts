// MUR SST — DailySSTData/MUR/mur_YYYYMMDD.csv
...
Deno.serve(async (req) => {
  try {
    ...
    if (!available.length) {
      return Response.json({ source: 'MUR', days: [] });
    }
    ...
    return Response.json({ source: 'MUR', days: validDays, generated_utc: new Date().toISOString() });
  } catch (error) {
