...
**`Getwinddata.py`** — the exception. **No `_REGION_CONFIGS`.** One shared grid (`LAT_MIN, LAT_MAX, LAT_STEP` / `LON_MIN, LON_MAX, LON_STEP`) with margin covers every region at once. Check whether the new region's bounds fall inside the current grid; if not, widen only the edge that's exceeded (don't shrink the others), and update the explanatory comment above the constants to mention the new region. **Widening this grid grows the output payload faster than it grows the grid itself** — `va_ri`'s widening (61x41=2501 points -> 69x59=4071, 1.6x) pushed `WindData/wind_latest.json` past ~40 MB (168 hours x each point stored twice: once compactly in `velocityJSON`, once verbosely as a `{lat,lon,u,v,speed}` object in `hours[].grid`). The script originally pushed this file to GitHub directly from Python via the Contents API's single-shot PUT, which rejected it with `422 "file is too large to be processed"`. Switching to the Git Data API's blob/tree/commit/ref sequence (GitHub's own recommended fix for that exact error) **did not actually solve it** — the create-blob call itself also 422'd at this payload size, confirmed in a subsequent production run. Both REST paths wrap the whole file as one inline base64 JSON request body, and neither tolerates a body this large in practice, regardless of GitHub's documented 100MB git object limit. **Fixed 2026-07-12 for real** by dropping both REST paths entirely: the script now just writes the file locally, and the workflow's own `git add`/`git commit`/`git push` (git's native smart-HTTP protocol, no JSON-body wrapping) handles getting it into the repo — the same pattern every other large/frequent data-writing workflow in this repo already uses (`Ocean_Mask.yml`, `fetch-ocean-dynamics.yml`, etc). **If a future region widens this grid further, don't reach for a bigger/different REST API call** — plain git push has no equivalent ceiling at any size this pipeline is likely to produce.
...
**Critical, learned the hard way on `va_ri` (2026-07-12): verify the committed `openocean_mask_<region>.json` was actually produced by running this exact script, not a substitute.** `va_ri`'s launch shipped a mask generated by some other ad-hoc "global_land_mask + morphological opening" process even though `bake_ocean_mask_va_ri.py` implementing the correct Natural Earth method existed in the repo the whole time — it just never got run. The symptom wasn't a crash or an obviously-wrong file; it looked like ordinary bay-bleed (altimetry color inside Chesapeake Bay, Delaware Bay, Long Island Sound, plus barrier-island lagoons misclassified as open ocean), which is *exactly* the kind of bug you'd expect from a correct mask too, since even accurate coastline data can't tell "enclosed bay" from "open ocean" on its own. Three separate rounds of hand-drawn exclusion-box patches (each verified with control points, each shipped, each still leaving real bleed and even clipping real open water at Cape May) failed to fully fix it, because the patches were being layered onto a categorically cruder base than every other live region uses. Only regenerating from the actual `bake_ocean_mask_<region>.py` method resolved several of the reported spots outright (no hand box needed) and made the remaining large-bay boxes far more reliable. **Before touching any altimetry bay-bleed report, `git log`/diff the region's own bake script against what's actually committed at `DailySSTData/<region>/ocean_mask.json`'s generation — don't assume the file was produced the way the repo's tooling says it should have been.**
...
```
`dataPathSuffix` drives all SST/CHL/altimetry/wind data URL construction (`""` = mid_atlantic root paths, any non-empty value = subdir). `wreckRegion` tags which named-region bucket a port belongs to in `wrecks.json` (used by the `sst-bottom-features` skill to organize research/CSV data) — safe to invent a new value per port even before any wreck data is tagged with it. **It no longer gates which bottom features a departure port can see**: since 2026-07-17 (commit d4b651c) the wrecks overlay shows every feature within the loaded map region's bounds regardless of the selected port's `wreckRegion` — the field is purely organizational now, not a visibility filter. `sstSeasonalDefaults` anchors the SST color ramp; if you don't have real measured seasonal ranges yet, estimate from latitude relative to existing regions and say clearly in a code comment that it's a placeholder pending calibration.
...
The selected region is stored two places, and both matter — a new region key must work in both or a signup can silently land on the wrong region:

1. **`auth.users.raw_user_meta_data.region`** — set by `supabase.auth.signUp({ options: { data: { region } } })` in the region step (`LandingPage.jsx`'s `AuthForm`, `SSTLive.jsx`'s `InlineLogin`). This is the value that actually determines the new user's region — see point 2.
2. **`user_profiles.region`** (plain `text` column, no enum/check constraint historically — confirm with a quick `.sql` grep before assuming this is still true) — populated by the `handle_new_user()` Postgres trigger (`AFTER INSERT ON auth.users`), which reads `NEW.raw_user_meta_data->>'region'` and validates it against a **hardcoded allowlist of the 4 known region keys** (`mid_atlantic`, `ga_sc`, `ne_fl`, `va_ri`), falling back to `mid_atlantic` if the value is missing or not in that list. **Adding a 5th region key means updating this trigger's allowlist too, or every signup picking the new region silently falls back to mid_atlantic** — this is not one of the files search-and-replaced elsewhere in this doc since it lives in the Supabase project (migration, not a repo file); check current definition with `SELECT prosrc FROM pg_proc WHERE proname = 'handle_new_user'` (or the Supabase MCP `execute_sql` tool) before assuming it's been kept in sync.

The client also does a post-confirmation `upsert({id, email, region}, {onConflict:'id'})` as a backstop (checkout-fallback signup path, and any pre-existing row) — see the Key Gotchas row on `user_profiles` upserts below if adding similar code: `email` must always be included, or the upsert 400s outright regardless of region.
...
- `SSTHeatmapLeaflet.jsx` / `SSTHeatmapMapbox.jsx` — reads `dataPathSuffix`, tries the region mask then falls back; VIIRS cache key includes region. **Altimetry's `setMaxBounds` specifically must use the real data bounds `gridToDataURL` computes, not the region's `llBounds`** — see the corrected Key Gotchas row below and `docs/map_viewport_nuances.md` section 8.6 (this doc previously said the opposite and was wrong, fixed 2026-07-13, commit `723b32d`)
- `src/hooks/useRegionAccess.js` — reads region from the Supabase profile dynamically
- **Not actually generalized, verify by hand:** no component remounts on a region change (`<AppShell region={userRegion}>` has no `key` prop) -- any `useEffect` with an empty/incomplete dependency array plus a `useRef` cache keyed only by date/id (not region) will silently serve a previous region's data after an in-tab region switch. This bit altimetry (`docs/map_viewport_nuances.md` section 8.5, fixed 2026-07-13) and hasn't been individually audited for CHL/SeaColor/composite/currents' equivalent fetch effects — check those too if a region-switch report comes in for a different layer.
...
**Viewport / auth:** map centers correctly, no grey bars, `setMaxBounds` allows panning to full coast, login routes correctly.

- [ ] Sign up fresh, pick the new region, confirm the email, log in — check `user_profiles.region` directly in the database (not just that the app *looks* right), since a 4th+ region key not yet in `handle_new_user()`'s allowlist (Step 3.2) falls back to `mid_atlantic` with zero visible error.
...
| Viewport locked to altimetry data edge, coast not pannable | Historical description of this row was backwards and caused a real regression — see the corrected row below | Do NOT use region config bounds (`llBounds`) for altimetry's `setMaxBounds`; SST/CHL/composite/seacolor legitimately use `llBounds` on non-tightened edges (their ocean-only fetch may not reach the inshore edge), but altimetry's grid is a full rectangle and must use its own real data bounds on all four edges |
| Altimetry alone lets the user pan past the region's real data edge (any direction) even on a fresh session with no prior region ever loaded — a still-blank strip renders solid altimetry color instead of basemap | Altimetry's overlay-build effect called `map.setMaxBounds(llBounds)` (full region bounds) instead of the real data bounds already computed by `gridToDataURL` for that same overlay; `dataBoundsRef.current` was never set for altimetry either, so later refits (vv-resize, post-sstReady) fell back to the same loose bound | Fixed 2026-07-13, commit `723b32d`: set `dataBoundsRef.current` from the real altimetry `{south,west,north,east}` and call `setMaxBounds` with that box instead of `llBounds`. Full writeup in `docs/map_viewport_nuances.md` section 8.6. Distinct from the cache bug (`b768948`, section 8.5) above — that one is about *what data loads*, this one is about *how far the user can pan* |
| Map pans to the previous region on region change | `selectedLocation` initialized once from the wrong region | `key={region}` on the relevant provider forces remount on region change -- **not actually applied**: `<AppShell region={userRegion}>` in `SSTLive.jsx` (~line 1312) still has no `key` prop as of 2026-07-13. A real instance of the underlying problem (a stale per-region cache surviving a region switch) surfaced for altimetry specifically -- see the row below and `docs/map_viewport_nuances.md` section 8.5 -- and was fixed narrowly (scope that one cache/effect by region) rather than by finally adding the remount. Other per-region caches in `SSTLive.jsx` likely share the same latent exposure; the remount fix would close all of them at once but costs a full cold-load of every layer on every region switch |
| Altimetry renders past the current region's real bounds (hard rectangular color cutoff well outside the region box), reproducible even in a clean incognito session in some cases | Two distinct causes seen for the same symptom: (1) `SSTLive.jsx`'s altimetry-discovery `useEffect` had an empty dependency array `[]` and its `_altimetryCache` (`useRef(new Map())`) was keyed only by date string, not region -- switching regions in the same tab without a reload reused another region's wider cached grid under the same date key; (2) confirm which is live before assuming (1): check the actual rendered `L.ImageOverlay`'s `getBounds()` in devtools/JS against the region's real data bbox -- a deploy-propagation delay can also transiently show stale behavior right after a fix ships | Fixed 2026-07-13, commit `b768948`: scope the cache key and the discovery effect's deps by `ALTIMETRY_BASE_R` (the region-scoped URL prefix). Full writeup in `docs/map_viewport_nuances.md` section 8.5. If this recurs, verify live via the same `getBounds()` JS check before re-diagnosing from scratch |
...
| Wind update workflow run completes all batches successfully but still fails, with `422 "file is too large to be processed"` in the log — **including after switching the write to the Git Data API** (blob/tree/commit/ref), which still 422'd on the create-blob call itself at this payload size | The wider grid's payload (168 hours x more points, each stored both compactly in `velocityJSON` and verbosely in `hours[].grid`) exceeded the practical size ceiling of *both* REST API paths (Contents API single-shot PUT, and Git Data API blob creation) — both wrap the whole file as one inline base64 JSON request body, which neither tolerates at this size regardless of GitHub's documented 100MB git object limit | Fixed 2026-07-12 for real: dropped both REST paths, script now writes the file locally and the workflow's own `git commit`/`git push` (native smart-HTTP, no JSON-body wrapping) handles the upload — same pattern as `Ocean_Mask.yml`/`fetch-ocean-dynamics.yml`. Don't trim the JSON payload format instead without also updating `SSTHeatmapLeaflet.jsx`, which parses `hours[].grid` as literal `{lat,lon,speed}` objects |
...
| New signup's region silently landed on `mid_atlantic` no matter what the user actually picked, on every single signup, with no error surfaced anywhere in the UI | Two stacked bugs, found 2026-07-15/16 (see `project_signup_region_bug_2026-07-15` memory for the full trace): (1) region was originally handed off via `sessionStorage`, which doesn't survive the email-confirmation link opening in a new tab — real bug, but not the actual blocker; (2) the client's `user_profiles` upsert only ever sent `{id, region}`. `user_profiles.email` is `NOT NULL` with no default, and Postgres validates `NOT NULL` columns on the INSERT branch of `INSERT ... ON CONFLICT DO UPDATE` even when the row already exists and the statement will just end up updating it — so this upsert 400'd on literally every call, silently (`.then(({error}) => console.warn(...))`, no user-visible failure). The `handle_new_user()` trigger's own hardcoded `mid_atlantic` default was therefore the only thing that ever actually landed | Fixed: region now travels in `signUp()`'s `user_metadata` (survives cross-device confirmation, unlike any client-side storage), `handle_new_user()` reads it directly at row-creation time (see Step 3.2 above) instead of hardcoding a default, and every `user_profiles` upsert that sets `region` now also includes `email`. **General lesson for any future partial upsert to `user_profiles` (or any table with a `NOT NULL`, no-default column): a payload missing a required column 400s the whole upsert regardless of `ON CONFLICT` — this is not an RLS problem and won't look like one.** When a Supabase write seems to have no effect, check `get_logs` (service `api`) for the actual HTTP status on the request before assuming RLS silently no-op'd it — RLS mismatches are silent 200s with 0 rows affected; constraint violations are 400s with a real error message the client may be swallowing |
