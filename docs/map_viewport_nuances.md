...
| Bottom-feature search: view zooms in, then visibly bounces back out; SST overlay looks like it didn't reload | Animated `flyTo` raced the SST/CHL data-layer effects' own `setMaxBounds`/`setMinZoom` re-enforcement (recomputed on every render) — the correction had a moving target to land on mid-animation instead of a settled view | Use synchronous `map.setView`, not `flyTo`, for any programmatic move that isn't the user's own pan/zoom — see section 9 |
| `leaflet.markercluster` throws "Map has no maxZoom specified" the moment Bottom Features builds its cluster layer | Map's `maxZoom` was only ever set inside `applyFillZoom` (delayed via double-rAF, permanently no-ops once `sstReadyRef.current` is true); on a fast/cache-warm load it could skip entirely, leaving `map.getMaxZoom()` at `Infinity` | Pass `maxZoom: 20` directly in the `L.map()` constructor options so the map is never in the unset state — see section 9 |

## 9. Bottom-feature search: `setView` vs `flyTo`, and the `maxZoom` production crash (2026-07-25)

Two related incidents from building the bottom-feature name-search tool (see `docs/map_control_panel.md`'s "Bottom feature name search" section for the feature itself).

### 9.1 `leaflet.markercluster` requires a finite `maxZoom`

Bottom Features renders wreck/structure markers through `L.markerClusterGroup` (`disableClusteringAtZoom: 13`, added `2ed7b3f`, 2026-07-22, before this session). That plugin throws `"Map has no maxZoom specified"` if built against a map whose `maxZoom` is unset (`Infinity`).

This map's `maxZoom` was only ever set inside `applyFillZoom` (see section 4) — and that function is a permanent no-op once `sstReadyRef.current` flips `true`. On a fast-loading (cache-warm) session, `applyFillZoom`'s double-rAF could be skipped or race past before ever running, leaving `map.getMaxZoom()` at `Infinity` for the whole session. Non-GL sessions were accidentally safe because the fallback CARTO tile layer's own `maxZoom: 19` gets adopted by Leaflet automatically; GL-basemap sessions (the common case) had nothing filling that role, so the crash hit the moment Bottom Features tried to build its cluster layer.

**Fix:** pass `maxZoom: 20` directly in the `L.map()` constructor options, so the map is never in the unset state regardless of whether `applyFillZoom` has run yet. `applyFillZoom`'s later `setMaxZoom(glLayerRef.current ? 20 : 12)` call still runs afterward and still narrows this to 12 in non-GL mode as before — this only fixes the window before that call.

**Note on diagnosing this class of bug:** the first pass at this fix incorrectly blamed `regionConfig.js`'s `maxZoom: 11` (a value the map doesn't actually read from that file) — always confirm which `maxZoom` a crash is actually about by grepping the live `L.map()` constructor and every `setMaxZoom` call site, not by assuming a config value is wired up somewhere it isn't.

### 9.2 Programmatic moves should use `setView`, not `flyTo`, when other effects can re-apply bounds mid-flight

The search feature originally used an animated `map.flyTo(...)` to bring a found result into view. This raced the SST/CHL/composite/seacolor data-layer effects (section 6), which recompute and call `setMaxBounds`/`setMinZoom` on every relevant render — and Leaflet auto-corrects (`_panInsideMaxBounds`, an implicit `setZoom` bump) any view that would land outside `maxBounds`/below `minZoom`, on *every* view change, animated or not (see section 6's note on `setMaxBounds` calling `_panInsideMaxBounds` immediately).

An animated `flyTo` gives that correction a moving target: it can land mid-flight, snapping the in-progress animation to a different position than the one it was headed toward. The visible result was a search result appearing to zoom in and then bounce back out to a wider view, and in the same window the SST overlay could look like it hadn't reloaded for the new position.

**Fix:** use `map.setView(latlng, zoom, { animate: false })` for this kind of programmatic move instead of `flyTo`. `setView` resolves synchronously — any bounds/zoom correction Leaflet applies happens inside the same call, before the calling code reads the map's settled state (e.g. `map.latLngToContainerPoint(...)` to position a UI element relative to a marker). There's no animation window for a competing effect to land inside. This trade-off (an instant jump instead of a smooth pan) was accepted deliberately after the animated version proved unreliable across several rounds of live testing — prefer `setView` for any future programmatic map move that isn't the user's own drag/pinch gesture, unless the animation is specifically requested and nothing else can plausibly re-touch bounds/zoom during it.
