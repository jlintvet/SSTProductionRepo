  ...
  useEffect(() => {
    ...
    const applyFillZoom = () => {
      try {
        ...
        const _cw = sz.x || 800;
        // Use visualViewport height directly — sz.y drifts as flex layout settles,
        // causing later timeouts to compute a different fillZoom and bounce.
        const _ch = Math.max(vpH * 0.75, 500);
    ...
    map.on("mousemove", (e) => {
      ...
      if (isWindMapRef.current && windDataRef.current?.hours?.length) {
    ...
    function handleTouch(e) {
      ...
      setHoverInfo({ px, py: py - 70, sst, depth_ft, chl: touchChl, color_class: touchColorClass, kd490: touchKd490,
        dist: refLoc ? distanceNm(refLoc.lat, refLoc.lon, lat, lon) : null,
        bearing: refLoc ? bearingDeg(refLoc.lat, refLoc.lon, lat, lon) : null,
      });
  ...
  useEffect(() => {
    ...
    const refit = () => {
      try {
        ...
        const sz = map.getSize();
        const vpH = window.visualViewport?.height || window.innerHeight || 0;
        const _cw = sz.x || 800, _ch = Math.max(vpH * 0.75, 500);
        const _mN = Math.log(Math.tan(Math.PI/4 + regionBounds.north*Math.PI/360));
        const _mS = Math.log(Math.tan(Math.PI/4 + regionBounds.south*Math.PI/360));
        const _mH = _mN - _mS, _lR = regionBounds.east - regionBounds.west;
  ...
  useEffect(() => {
    ...
    sp.zones.forEach(zone => {
      ...
      function drawCircle() {
        ...
        function seededRand(i) { const x = Math.sin(seed + i * 127.1 + 311.7) * 43758.5453; return x - Math.floor(x); }
        const wX = Math.min(W, H) * 0.055;
        const perim = [];
        for (let i = 0; i < N; i++) {
          const t = (i / N) * n, i0 = Math.floor(t) % n, i1 = (i0 + 1) % n, f = t - Math.floor(t);
          perim.push([localPts[i0][0] * (1 - f) + localPts[i1][0] * f, localPts[i0][1] * (1 - f) + localPts[i1][1] * f]);
        }
        const wobbly  = perim.map(([x, y], i) => { const r = (seededRand(i) - 0.5) * 2 * wX, th = seededRand(i + 100) * Math.PI * 2; return [x + r * Math.cos(th), y + r * Math.sin(th)]; });

        svgEl.innerHTML = `
          <path d="${catmullRomToBezier(wobbly)}" fill="${color}" fill-opacity="0.10" stroke="${color}" stroke-opacity="0.75" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
        `;
      }

      svgOverlay.addTo(lyr);
      requestAnimationFrame(drawCircle);
      drawHandlers.push(drawCircle);
      map.on("zoomend moveend", drawCircle);
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-full p-0 overflow-hidden w-full">
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        <div className="relative bg-slate-100 flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          {/* Collapse toggle */}
          <button className="hidden sm:flex absolute items-center justify-center bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg shadow-sm hover:bg-slate-50 transition-colors"
            style={{ right: 8, top: 8, width: 26, height: 26, zIndex: 501, padding: 0 }}
            onClick={() => setPanelCollapsed(c => !c)}
            title={panelCollapsed ? "Show controls" : "Hide controls"}>
            {panelCollapsed
              ? <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#0e7490" strokeWidth="1.5" strokeLinecap="round"><line x1="2" y1="4" x2="14" y2="4"/><line x1="2" y1="8" x2="14" y2="8"/><line x1="2" y1="12" x2="14" y2="12"/><circle cx="5" cy="4" r="1.5" fill="#0e7490" stroke="none"/><circle cx="10" cy="8" r="1.5" fill="#0e7490" stroke="none"/><circle cx="6" cy="12" r="1.5" fill="#0e7490" stroke="none"/></svg>
              : <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M3.5 2L7.5 6L3.5 10" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            }
          </button>

          {/* Pan + Inspect quick-access when collapsed */}
          {panelCollapsed && (
            <div className="hidden sm:flex absolute flex-col gap-1" style={{ right: 8, top: 42, zIndex: 501 }}>
              <button onClick={() => setInteractionMode("pan")} title="Pan mode"
                ...
                style={{ width:26, height:26, padding:0, borderColor:interactionMode==="pan"?"#334155":undefined, background:interactionMode==="pan"?"#334155":undefined }}>
                <Move className={`w-3.5 h-3.5 ${interactionMode==="pan"?"text-white":"text-slate-500"}`}/>
              </button>
              <button onClick={() => setInteractionMode("crosshair")} title="Inspect mode"
                className="flex items-center justify-center bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg shadow-sm hover:bg-slate-50 transition-colors"
                style={{ width:26, height:26, padding:0, borderColor:interactionMode==="crosshair"?"#0891b2":undefined, background:interactionMode==="crosshair"?"#0891b2":undefined }}>
                <Crosshair className={`w-3.5 h-3.5 ${interactionMode==="crosshair"?"text-white":"text-slate-500"}`}/>
              </button>
            </div>
          )}
            ...
            <div className="sm:hidden fixed left-0 right-0 bg-white border-t border-slate-200 shadow-xl flex flex-col" style={{bottom: 0, zIndex:2000, maxHeight:"50vh", overflowY:"auto"}}>
              ...
              <div className="flex-1 overflow-y-auto p-2">
                <SavedLocations locations={savedLocations} onRefresh={fetchSavedLocations}
                  ...
                  highlightedId={highlightedLocation?.id} onShare={onShare}/>
              </div>
            </div>
          )}
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {[
                        ...
                        { label: "Blend",    active: activeDataLayer === "sst" && dataSource === "GOESCOMP", fn: () => { setActiveDataLayer("sst"); setDataSource("GOESCOMP"); } },
                ...
                {mobilePanel === "wind" && (
                  <>
                    ...
                    <button onClick={() => setShowWindOverlay(v => !v)}
                      className={`w-full text-[11px] font-semibold px-3 py-2 rounded-lg border flex items-center gap-1.5 transition-colors ${showWindOverlay ? "bg-sky-600 text-white border-sky-600" : "bg-white text-slate-600 border-slate-300"}`}>
                      <Wind className="w-3.5 h-3.5" />{windLoading ? "Loading…" : showWindOverlay ? "Wind overlay on" : "Wind overlay"}
                    </button>
                    <button onClick={() => { setActiveDataLayer(isWindMap ? "sst" : "windmap"); }}
                      className={`w-full text-[11px] font-semibold px-3 py-2 rounded-lg border flex items-center gap-1.5 transition-colors ${isWindMap ? "bg-sky-700 text-white border-sky-700" : "bg-white text-slate-600 border-slate-300"}`}>
            ...
            <div className="absolute bg-white/95 border border-slate-200 rounded-lg text-xs shadow-lg"
              ...
              <div className="relative px-2.5 py-1.5 space-y-0.5">
                ...
                <div className={touchMarker ? "pr-6" : ""}>
                  ...
                  {activeDataLayer==="windmap"&&hoverInfo.windSpeed_kt!=null&&<div className="text-sky-600 font-semibold">{hoverInfo.windSpeed_kt.toFixed(1)} kt</div>}
                  {activeDataLayer==="windmap"&&hoverInfo.windDir_deg!=null&&<div className="text-sky-500 font-medium">{Math.round(hoverInfo.windDir_deg)}° {bearingLabel(hoverInfo.windDir_deg)}</div>}
                  ...
                  {hoverInfo.sst==null&&hoverInfo.depth_ft==null&&hoverInfo.chl==null&&hoverInfo.kd490==null&&hoverInfo.windSpeed_kt==null&&<div className="text-slate-400">No data</div>}
                </div>
              </div>
            </div>
          )}
          ...
          {hotspotPopup && (() => {
            ...
            return (
              ...
              >
                {/* Header row: close + optional warning icon */}
          ...
          <div className="sm:hidden absolute left-0 right-0 px-2" style={{ bottom: windActive ? 112 : 64, zIndex: 600, pointerEvents: "auto" }}>
            {isWindMap
              ? null
